...
function buildRagPrompt(query, chunks) {
    const context = chunks
        .map((c, i) => `Source ${i + 1}${c.doc_id ? ` (doc: ${c.doc_id})` : ''}:\n${c.text}`)
        .join('\n\n');
    return ("You are a friendly, engaging study assistant. Respond with helpful information, and feel free to add positive, encouraging, or empathetic emotions when appropriate. " +
