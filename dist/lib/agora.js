...
exports.agoraSTT = agoraSTT;
exports.agoraTTS = agoraTTS;
exports.createAgoraSession = createAgoraSession;
exports.startAgoraAgent = startAgoraAgent;
...
async function agoraSTT(audioBuffer) {
...
async function agoraTTS(text, opts) {
    if (!CUSTOMER_ID || !CUSTOMER_SECRET)
        throw new Error('Missing Agora customer credentials');
    ...
    if (!url) {
        if (!APP_ID)
            throw new Error('AGORA_APP_ID required to compose speak URL');
        if (!AGORA_AGENT_ID && !process.env.AGORA_AGENT_ID)
            throw new Error('AGORA_AGENT_ID required to compose speak URL');
    ...
    if (typeof opts?.interruptable === 'boolean')
        body.interruptable = opts.interruptable;
    const res = await fetch(url, {
        method: 'POST',
        headers: {
...
async function startAgoraAgent(opts) {
    if (!CUSTOMER_ID || !CUSTOMER_SECRET)
        throw new Error('Missing Agora customer credentials');
    if (!APP_ID)
...
function buildRtcToken(appId, appCertificate, channelName, uid, expireTimestamp) {
    ...
    const writeUint16LE = (n) => { const b = Buffer.allocUnsafe(2); b.writeUInt16LE(n, 0); return b; };
    const writeUint32LE = (n) => { const b = Buffer.allocUnsafe(4); b.writeUInt32LE(n, 0); return b; };
    // pack string with length prefix (uint16)
    const packString = (buf) => Buffer.concat([writeUint16LE(buf.length), buf]);
    ...
    const signature = crypto_1.default.createHmac('sha256', appCertificate).update(body).digest();
